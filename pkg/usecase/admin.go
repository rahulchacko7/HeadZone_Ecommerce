...
func (ad *adminUseCase) SalesByDate(dayInt int, monthInt int, yearInt int) ([]models.OrderDetailsAdmin, error) {
	...
	if monthInt > 0 && monthInt <= 12 && time.Month(monthInt) == currentMonth {
		body, err := ad.adminRepository.SalesByMonth(monthInt)
		if err != nil {
			return []models.OrderDetailsAdmin{}, err
		}
	...
	if dayInt > 0 && dayInt <= 31 && dayInt == currentDay {
		body, err := ad.adminRepository.SalesByDay(dayInt)
		if err != nil {
			return []models.OrderDetailsAdmin{}, err
		}
	...
	return []models.OrderDetailsAdmin{}, errors.New("no data found")
}
